import React, { Component } from 'react';
import {
  Text,
  View,
  TouchableOpacity,
  Image
} from 'react-native';

import { createStackNavigator } from 'react-navigation'

import { Provider } from 'react-redux'
import { createStore } from 'redux'

import ScheduleScreen from './ScheduleScreen';
import AddTaskScreen from './AddTaskScreen';
import { gray, white, calendarHighlight } from '../styles/styles';

import rootReducer from '../reducers' // tu dong pointer vao file index (js default)

const store = createStore(rootReducer)

const Navigation = createStackNavigator({
  Schedule: {
    screen: ScheduleScreen,
    navigationOptions: ({ navigation }) => ({
      title: 'Schedule',
      headerRight:
        <TouchableOpacity onPress={() => navigation.navigate('AddTask')}>
          <Image style={{ width: 30, height: 30, marginEnd: 10 }}
            source={{ uri: 'data:image/png;base64,' }} />
        </TouchableOpacity>,
      headerTitleStyle: {
        fontSize: 22,
        color: gray
      },
      headerStyle: {
        backgroundColor: white,
        borderBottomWidth: 0
      }
    })
  },
  AddTask: {
    screen: AddTaskScreen,
    navigationOptions: ({ navigation }) => ({
      title: 'Add New Task',
      headerLeft:
        <TouchableOpacity onPress={() => navigation.navigate('Schedule')}>
          <Image style={{ width: 25, height: 25, marginStart: 5 }}
            source={{ uri: 'data:imsage/png;base64,' }} />
        </TouchableOpacity>,
      headerRight:
        <TouchableOpacity
          onPress={() => navigation.navigate('Schedule')}>
          <Text style={{
            marginEnd: 10,
            fontWeight: 'bold',
            fontSize: 18,
            color: calendarHighlight
          }}>Done</Text>
        </TouchableOpacity>
      ,
      headerTitleStyle: {
        fontSize: 22,
        color: gray
      },
      headerStyle: {
        backgroundColor: white,
        borderBottomWidth: 0 // not working
      }
    })
  }
})

class App extends Component {
  state = {}
  render() {
    return (
      <Provider store={store}>
        <Navigation />
      </Provider>
    );
  }
}

export default App;